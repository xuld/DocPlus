...
namespace DocPlus.Javascript {
    ...
    /// 解析 Java 风格的文档注释的工具。
    /// </summary>
    public sealed class JavaCommentParser1 {
        ...
        /// </summary>
        public JavaCommentParser1(DocProject project) {
            _project = project;
            AddDefaultTypes();
            InitTags();
        ...
        /// 用于快速按名字获取标签的缓存。
        /// </summary>
        Dictionary<string, CommentNode1> _commentNodes = new Dictionary<string, CommentNode1>();

        /// <summary>
        /// 存放各个标签的处理函数。
        /// </summary>
        Dictionary<CommentNode1, Action<Variant, CommentNode1>> _actions = new Dictionary<CommentNode1, Action<Variant, CommentNode1>>();
        ...
        void AddSystemTag(CommentNode1 tag, Action<Variant, CommentNode1> action) {
        ...
        CommentNode1 GetCommentTags(string name) {
            if (_commentNodes.ContainsKey(name))
                return _commentNodes[name];

           CommentNode1 t = _commentNodes[name] = new CommentNode1(name);
            _actions[t] = Warning;
            return t;
        }
        ...
        void InitTags() {
            ...
            AddSystemTag(CommentNode1.Public, (v, c) => ParseMemberAccess(v, MemberAccess.Public));
            AddSystemTag(CommentNode1.Private, (v, c) => ParseMemberAccess(v, MemberAccess.Private));
            AddSystemTag(CommentNode1.Protected, (v, c) => ParseMemberAccess(v, MemberAccess.Protected));
            AddSystemTag(CommentNode1.Internal, (v, c) => ParseMemberAccess(v, MemberAccess.Internal));

            AddSystemTag(CommentNode1.Final, (v, c) => ParseMemberAttributes(v, MemberAttributes.Sealed));
            AddSystemTag(CommentNode1.Static, (v, c) => ParseMemberAttributes(v, MemberAttributes.Static));
            AddSystemTag(CommentNode1.Abstract, (v, c) => ParseMemberAttributes(v, MemberAttributes.Abstract));
            AddSystemTag(CommentNode1.Virtual, (v, c) => ParseMemberAttributes(v, MemberAttributes.Virtual));
            AddSystemTag(CommentNode1.Const, (v, c) => ParseMemberAttributes(v, MemberAttributes.Const));
            AddSystemTag(CommentNode1.ReadOnly, (v, c) => ParseMemberAttributes(v, MemberAttributes.ReadOnly));
            AddSystemTag(CommentNode1.Override, (v, c) => ParseMemberAttributes(v, MemberAttributes.Override));
        ...
        void DefineComment(string fromName, string toName) {
            _commentNodes[fromName] = _commentNodes.ContainsKey(toName) ? _commentNodes[toName] : new CommentNode1(fromName);
        }
        ...
        void ParseCommentTag(Variant currentVariant, CommentNode1 node) {
        ...
        public Variant Parse(TokenInfo token) {
            ...
            // 目前属于的标签。默认是 Summary
            CommentNode1 current = CommentNode1.Summary;
